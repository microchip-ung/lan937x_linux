...
static void phylink_mac_initial_config(struct phylink *pl, bool force_restart)
{
	...
	case MLO_AN_PHY:
		pr_info("phylink_mac_initial_config:MLO_AN_PHY");
		link_state = pl->phy_state;
		break;

	case MLO_AN_FIXED:
		pr_info("phylink_mac_initial_config:MLO_AN_FIXED");
		phylink_get_fixed_state(pl, &link_state);
		break;

	case MLO_AN_INBAND:
		pr_info("phylink_mac_initial_config:MLO_AN_INBAND");
...
static void phylink_resolve(struct work_struct *w)
{
	...
	if (pl->phylink_disable_state) {
		...
		case MLO_AN_PHY:
			link_state = pl->phy_state;
			phylink_apply_manual_flow(pl, &link_state);
			pr_info("phylink_resolve:MLO_AN_PHY & mac_config_up");
			phylink_mac_config_up(pl, &link_state);
			break;

		case MLO_AN_FIXED:
			phylink_get_fixed_state(pl, &link_state);
			pr_info("phylink_resolve:MLO_AN_FIXED & mac_config_up");
			phylink_mac_config_up(pl, &link_state);
			break;

		case MLO_AN_INBAND:
			pr_info("phylink_resolve:MLO_AN_INBAND");
			       ...
			       const struct phylink_mac_ops *mac_ops)
{
	...
	bitmap_fill(pl->supported, __ETHTOOL_LINK_MODE_MASK_NBITS);
	linkmode_copy(pl->link_config.advertising, pl->supported);
	pr_info("phylink_validate");
	phylink_validate(pl, pl->supported, &pl->link_config);

	ret = phylink_parse_mode(pl, fwnode);
	if (ret < 0) {
		pr_info("phylink_parse_mode failed");
		kfree(pl);
		return ERR_PTR(ret);
	}
	...
	if (pl->cfg_link_an_mode == MLO_AN_FIXED) {
		...
		if (ret < 0) {
			pr_info("phylink_parse_fixedlink failed");	
			kfree(pl);
			return ERR_PTR(ret);
		}
	}
	...
	if (ret < 0) {
		kfree(pl);
		pr_info("phylink_register_sfp failed");
		return ERR_PTR(ret);
	}
	pr_info("phylink_create End,%d",pl->link_config.an_enabled);
	return pl;
}
...
int phylink_connect_phy(struct phylink *pl, struct phy_device *phy)
{
	...
	pr_info("phylink_connect_phy: phy_attach_direct is called");	
	ret = phylink_attach_phy(pl, phy, pl->link_interface);
	if (ret < 0)
		return ret;
	pr_info("phylink_connect_phy: phylink_bringup_phy is called");
			   ...
			   u32 flags)
{
	...
	ret = phy_attach_direct(pl->netdev, phy_dev, flags,
				pl->link_interface);
	pr_info("phylink_of_phy_connect: phy_attach_direct is called");			
	if (ret)
		return ret;
	pr_info("phylink_bringup_phy call");
