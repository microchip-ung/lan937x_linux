...
static int dsa_slave_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	pr_info("dsa_slave_change_mtu");
	pr_info("dsa_slave_change_mtu numports:%d", ds->num_ports);

	for (i = 0; i < ds->num_ports; i++) {
		int slave_mtu;
		pr_info("dsa_slave_change_mtu current port i:%d", i);
		if (!dsa_is_user_port(ds, i))
			continue;
		pr_info("dsa_is_user_port after user port");
		...
		pr_info("port num:%d, largestmtu before :%d", i, largest_mtu);
		pr_info(" port:%d, slave mtu :%d", i, slave_mtu);

		if (largest_mtu < slave_mtu)
			largest_mtu = slave_mtu;
		
		pr_info("port num:%d, largestmtu after :%d", i, largest_mtu);
	}

	cpu_dp = dsa_to_port(ds, port)->cpu_dp;
	
	pr_info("master->max_mtu : %d", master->max_mtu);
	pr_info("dev->max_mtu : %d", dev->max_mtu);

	mtu_limit = min_t(int, master->max_mtu, dev->max_mtu);
	old_master_mtu = master->mtu;
	new_master_mtu = largest_mtu + cpu_dp->tag_ops->overhead;
	pr_info("cpu_dp->tag_ops->overhead : %d", cpu_dp->tag_ops->overhead);
	pr_info("new_master_mtu : %d, mtu_limit:%d", new_master_mtu, mtu_limit);
	if (new_master_mtu > mtu_limit)
		return -ERANGE;
	pr_info("reached here after ERANGE");
	...
	if (new_master_mtu != old_master_mtu) {
		...
		pr_info("dev_set_mtu inside if called");
		if (err < 0)
		...
		pr_info("dsa_port_mtu_change inside if called");
		if (err)
			goto out_cpu_failed;
	}

	err = dsa_port_mtu_change(dp, new_mtu, false);
	pr_info("dsa_port_mtu_change2 called");
	if (err)
	...
	pr_info("dsa_bridge_mtu_normalization called");
...
out_port_failed:
	if (new_master_mtu != old_master_mtu) {
		dsa_port_mtu_change(cpu_dp, old_master_mtu -
				    ...
				    true);
		pr_info("out_port_failed called");
	}
out_cpu_failed:
	if (new_master_mtu != old_master_mtu) {
		dev_set_mtu(master, old_master_mtu);
		pr_info("out_cpu_failed called");
	}
out_master_failed:
	pr_info("out_master_failed called");
	return err;
}
...
static int dsa_slave_phy_setup(struct net_device *slave_dev)
{
	...
	pr_info("phylink_of_phy_connect call");
