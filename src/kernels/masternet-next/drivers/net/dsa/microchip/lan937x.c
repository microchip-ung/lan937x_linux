				...
				const unsigned char *addr, u16 vid)
{
	...
	pr_info("fdb del,p:%d,vid:%x",port,vid);
...
static u32 lan937x_tx_phy_bank_read(struct ksz_device *dev, int port, u8 bank, u8 reg)
{
    ...
    /* Keep test_mode bit always to not disable DSP access. */
	ctrl |= TX_TEST_MODE;
