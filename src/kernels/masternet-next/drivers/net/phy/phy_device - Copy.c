...
int phy_init_hw(struct phy_device *phydev)
{
	...
	/* Deassert the reset signal */
	phy_device_reset(phydev, 0);
	pr_info("phy_device_reset completed");
	if (!phydev->drv)
		return 0;
	pr_info("phy_device_reset no error");
	if (phydev->drv->soft_reset) {
		pr_info("soft_reset inside if");
	...
	if (ret < 0)
		return ret;
	pr_info("phy_device_soft_reset completed");
	ret = phy_scan_fixups(phydev);
	pr_info("phy_scan_fixups");
	if (ret < 0)
		return ret;
	pr_info("before phydev->drv->config_init");
	if (phydev->drv->config_init){
		ret = phydev->drv->config_init(phydev);
		pr_info("called phydev->drv->config_init");
	}
		      ...
		      u32 flags, phy_interface_t interface)
{
	 ...
	 */
	pr_info("phy_init_hw call");
	err = phy_init_hw(phydev);
	if (err) {
		goto error;
		pr_info("Error: phy_init_hw");
	}
